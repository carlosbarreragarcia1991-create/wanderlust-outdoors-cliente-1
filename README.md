...
5 páginas listas para subir a **GitHub Pages** (o cualquier hosting estático):

```
wanderlust-outdoors/
├── index.html       → 1. Landing / Inicio
├── carrito.html      → Carrito de compra (varios artículos a la vez)
...
## 🛒 Carrito de compra
- Cada botón **"Añadir al carrito"** (tienda y expediciones) llama a `añadirAlCarrito(id, nombre, precio, tipo)` en `js/script.js`, que guarda el artículo en `localStorage` (clave `wanderlust_carrito`) y dispara el evento `add_to_cart` al `dataLayer`.
- El icono 🛒 con contador se inyecta automáticamente en el header de **todas** las páginas — no hace falta tocar el HTML de cada una.
- `carrito.html` permite ajustar cantidades, eliminar artículos y ver el total antes de pasar al registro. Dispara `view_cart` y `remove_from_cart`.
- `checkout.html` y `gracias.html` ahora leen el carrito completo (no un solo artículo por URL), calculan el total y disparan `add_payment_info` / `purchase` con **todos** los artículos.
- Al completar la compra, el carrito se vacía automáticamente.

## 💬 CTA de WhatsApp
- Botón flotante (visible en las 5 páginas, inyectado por JS) + botón destacado en el hero de `index.html`.
- **Importante:** sustituye el número de prueba `34600000000` por tu número real (con prefijo de país, sin `+` ni `00`) en la constante `WHATSAPP_NUMERO` al principio de `js/script.js`.
- Cada clic dispara el evento `contacto_whatsapp` al `dataLayer` (con el origen: `"hero"` o `"flotante"`), para que puedas medirlo como conversión en GTM/GA4 si quieres.

## Cómo funciona el "embudo" (funnel)
- `index.html` → "Añadir al carrito" (tienda/expediciones) guarda el artículo y avisa visualmente → el usuario puede seguir añadiendo o ir a **carrito.html**.
- `carrito.html` → revisa cantidades y total → **registro.html**.
- `registro.html` → al enviar el formulario, se guarda el lead y se dispara el evento `generate_lead` → redirige a **checkout.html**.
- `checkout.html` → pago simulado sobre el carrito completo → dispara `add_payment_info` → redirige a **gracias.html**.
- `gracias.html` → dispara el evento `purchase` con el importe total y todos los artículos, y vacía el carrito. **Esta es la página de conversión.**
